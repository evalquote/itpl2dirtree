...
int o_dry = 0;
int o_m3u = 0;      // m3u 形式 効果なし
int o_check = 0;    // track にあるが,  playlist にない
int o_verbose = 0;
int o_debug = 0;
char *o_path = "";

const char *rmprefix = "file:///Users/amagai/Music/iTunes/iTunes%20Media/";
...
void
command(const char *folder, const char *pname, FILE *plfd, int seq, struct _track *rp)
{
  ...
  char *lc = strdup(loc);
  char *sp = lc;
  char *ep = lc + strlen(rmprefix); // skip prefix
  while (*ep) {
    if (*ep == '%') {  // %XY -> char(0xXY)
      *sp++ = deesc2(++ep);
      ep += 2;
  ...
  *sp = '\0';
  ...
  if (plfd) {
    char *name = eschy(rp->name, "-");
    // char *path = eschy(sl, " ");
    char *path = sl;
    if (rp->artist) {
      char *artist = eschy(rp->artist, "-");
      fprintf(plfd, "#EXTINF:%d, %s - %s\n%03d_%s\n",
	      rp->totaltime, name, artist, seq, path);
      free(artist);
    } else {
      fprintf(plfd, "#EXTINF:%d, %s - \n%03d_%s\n",
	      rp->totaltime, name, seq, path);
    }
    // free(path);
    free(name);
  } else {
    fprintf(stderr, "command null plfd\n");
  }
...
end_dict(struct _ud *udp)
{
  ...
  if (st_playlists == 1) {
	...
	if (ret < 0) fprintf(stderr, "ntrackHash inc %d\n", ret);
	  ...
	  command(fp, atp->name, atp->plfd, atp->plseq++, rp);
	} else {
